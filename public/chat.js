...
// —— 关键：通用提取文本工具 ——
// 处理 GPT-OSS 的 delta.content：可能是 string 或对象({value|text|content}…)
function extractTextFromDeltaContent(content) {
  ...
  // 非文本类型一律忽略，避免出现 [object Object]
  return "";
}

// 统一从一条 SSE JSON 里提取追加文本
function pickChunkText(jsonData) {
  // Llama/DeepSeek/Workers AI 标准
  ...
  // 某些实现会有 ch.text 或 ch.message.content
