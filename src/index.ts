...
async function handleChatRequest(
  ...
  try {
    ...
    // Create a transform stream to convert the AI response to SSE format
        ...
        if (text.includes('data: ')) {
          ...
          for (const line of lines) {
            try {
              ...
              if (data.response) {
                controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify({ response: data.response })}\n\n`));
              } else if (data.choices?.[0]?.delta?.content) {
                controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify({ response: data.choices[0].delta.content })}\n\n`));
              }
            } catch {
              // If not JSON, treat as plain text
              if (line.trim()) {
                controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify({ response: line })}\n\n`));
              }
            }
          }
        }
